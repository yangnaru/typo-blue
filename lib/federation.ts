import { createFederation, Person, Article, Note, Follow, Accept, Reject, MemoryKvStore, InProcessMessageQueue } from "@fedify/fedify";
...
federation.setOutboxDispatcher("/@{identifier}/outbox", async (ctx, identifier, cursor) => {
  ...
  const posts = await db.query.post.findMany({
    where: (post, { eq, and, isNotNull }) => and(
...
federation.setFollowersDispatcher("/@{identifier}/followers", async (ctx, identifier, cursor) => {
  // Fetch followers from database
  const followers = await db
    .select()
    .from(activityPubFollow)
    .innerJoin(activityPubRemoteActor, eq(activityPubFollow.remoteActorId, activityPubRemoteActor.id))
    .where(and(
...
federation.setFollowingDispatcher("/@{identifier}/following", async (ctx, identifier, cursor) => {
