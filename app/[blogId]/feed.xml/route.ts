...
export async function GET(req: NextRequest, { params }: { params: { blogId: string } }) {
    ...
    const root = create({ version: '1.0', encoding: 'utf-8' })
        ...
        .ele('id').txt(`${url}/@${blog.slug}/feed.xml`).up()
        .ele('title', { type: 'html' }).txt(blog.name || handle).up()
        .ele('subtitle').txt(blog.description || '').up();
    ...
    for (const post of blog.posts) {
        ...
        entry.ele('author').ele('name').txt(blog.name || handle);
        entry.ele('link', { rel: 'alternate', href: `${url}/@${blog.slug}/${postSlug}`, type: 'text/html', title: post.title });
